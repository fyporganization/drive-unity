...
export interface OneDriveImageRAGInput {
  user_id: string;
  one_drive_account_id: string;
}

export interface OneDriveImageRAGResult {
  message: string;
  total_images: number;
  indexed: number;
  reused: number;
...
export async function indexImagesForOneDriveAccount(
  input: OneDriveImageRAGInput
): Promise<OneDriveImageRAGResult> {
  ...
  return {
    message: 'OneDrive image RAG pipeline complete',
    total_images: total,
    indexed,
    reused,
    skipped,
